...
export default {
  ...
  methods: {
    ...
    handleNextStep: function() {
      // this.$router.push('/stepIndex/step' + (this.step + 1));
      this.$router.push({
        name:'step' + (this.step + 1),
        // path:'/stepIndex/step' + (this.step + 1),
        query:{'ids':this.step+1,'name':'step'}
      });
      var _this = this;
      setTimeout(function() {
        if (_this.isRouter) {
    ...
    handlePublish: function() {
      var tmp=store.store.state;
      alert(tmp.step1Form.name+'/'+tmp.step2Form.name+'/'+tmp.step3Form.name+'/'+tmp.step4Form.name);
